...
const ProgressComp = () => {
  ...
  async function deleteData(delid) {
    try {
      await axios.delete(
        `${API_URL}/progress/deleteProgress/${delid}`,
        {
          withCredentials: true,
        }
      );
  ...
  const ProgressRing = ({ percentage, label, index }) => {
    ...
    const strokeDashoffset = circumference - (formattedPercentage / 100) * circumference;
    ...
    <>
      <div className="fitness-analytics">
        <div className="container">
          <Link to="/add-progress" className="btn-default float-end">
            Add Progress
          </Link>
